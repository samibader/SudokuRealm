...
var options = {
    ...
    highlightnum: function (event) {
        ...
        var value = (c != null ? (c == 120 || c == 88) ? "X" : c - 48 : $(this).text());
        if (value != "X") {
            ...
            $('.shallowcandidates tr td').each(function () {
                if ($(this).text() == value)
                    $(this).addClass('cellhighlighted');
            });
        }
        else {
            that.removeHighlight();
...
var board = {
    ...
    restart: function () {
        ...
        for (var i = 0; i < 21; i++) {
            tempArray = [];
            for (var j = 0; j < 21; j++) {
                tempArray.push({ value: that.string_data[j + i * 21], possibilities: that.array_data[i][j].originalPossibilities, originalPossibilities: that.array_data[i][j].originalPossibilities, isfixed: that.array_data[i][j].isfixed, notes: [] });
            }
    ...
    transformToArray: function (str) {
        ...
        for (var i = 0; i < 21; i++) {
            ...
            for (var j = 0; j < 21; j++) {
                ...
                tempArray.push({ value: str[j + i * 21], possibilities: possibilities, originalPossibilities: originalPossibilities, isfixed: false, notes: [] });
            }
            result.push(tempArray);
        }
        return result;
            ...
            for (var j = 0; j < 21; j++) {
                ...
                if (this.array_data[i][j].value == '0') {
                    ...
                    for (var val = 1; val < 10; val++) {
                        if (!board.valueIsInRow(i, j, val) && !board.valueIsInColumn(i, j, val) && !board.valueIsInBlock(i, j, val)) {
        ...
        for (var i = 0; i < 21; i++) {
            for (var j = 0; j < 21; j++) {
                if (!that.array_data[i][j].isfixed) {
                    ...
                    for (var val = 1; val < 10; val++) {
                        if (!that.valueIsInRow(i, j, val) && !that.valueIsInColumn(i, j, val) && !that.valueIsInBlock(i, j, val)) {
                            this.array_data[i][j].possibilities.push(val);
                        }
                    }
                }
    ...
    removeHighlight: function () {
        ...
        //            }
        //        }
        $('.cellhighlighted').each(function () {
            $(this).removeClass('cellhighlighted');
        });
    },
    ...
    valueIsInRow: function (rowIndex, colIndex, value) {
        var that = board;
        // note: the position is in first sudoku 0*0 -> 8*8
        if (rowIndex < 9 && colIndex < 9) {
            // check to see if the cell is shared or not 
            if (rowIndex >= 6 && colIndex >= 6) {
                // the cell is in a shared position
                for (var j = 0; j < 15; j++) {
                ...
                // the cell is NOT in a shared position
                for (var j = 0; j < 9; j++) {
                    if (that.array_data[rowIndex][j].value == value)
                        return true;
                }
        ...
        // note: the position is in second sudoku 0*12 -> 8*20
        else if (rowIndex < 9 && colIndex >= 12) {
            // check to see if the cell is shared or not 
            if (rowIndex >= 6 && colIndex <= 14) {
                // the cell is in a shared position
                for (var j = 6; j < 21; j++) {
                    if (that.array_data[rowIndex][j].value == value)
                        return true;
        ...
        // note: the position is in Third sudoku 12*0 -> 20*8
        else if (rowIndex >= 12 && colIndex <= 8) {
            // check to see if the cell is shared or not 
            if (rowIndex <= 14 && colIndex >= 6) {
                // the cell is in a shared position
                for (var j = 0; j < 15; j++) {
                    if (that.array_data[rowIndex][j].value == value)
                        return true;
        ...
        // note: the position is in Forth sudoku 12*12 -> 20*20
        else if (rowIndex >= 12 && colIndex >= 12 && colIndex <= 20) {
            // check to see if the cell is shared or not 
        ...
        // note: the position is in Fifth sudoku 6*6 -> 14*14
        else {
            // the cell is NOT in a shared position
            for (var j = 6; j < 15; j++) {
                if (that.array_data[rowIndex][j].value == value)
    ...
    valueIsInColumn: function (rowIndex, colIndex, value) {
        //                var that = board;
        //                for (var i = 0; i < 9; i++) {
