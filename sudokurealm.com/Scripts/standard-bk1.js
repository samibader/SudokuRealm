...
var board = {
    ...
    restart: function () {
        ...
        for (var i = 0; i < 9; i++) {
            tempArray = [];
            for (var j = 0; j < 9; j++) {
                tempArray.push({ value: that.string_data[j + i * 9], possibilities: that.array_data[i][j].originalPossibilities, originalPossibilities: that.array_data[i][j].originalPossibilities, isfixed: that.array_data[i][j].isfixed, notes: [] });
            }
    ...
    countEmptyCells: function () {
        ...
        for (var i = 0; i < 9; i++) {
            for (var j = 0; j < 9; j++) {
                if (that.array_data[i][j].value == '0' || that.array_data[i][j].value == '?')
                    counter++;
            }
        }
        that.emptyCellsCount = counter;
        if (counter == 0) {
            that.checkSolution();
        }
    },
    ...
    writeConsole: function () {
        ...
        var str = "";
        $div.empty();
        $span.text('full:' + board.emptyCellsCount);
        $div.append($span);
        $div.append($hr);
        $hr = $("<hr/>");
        for (var i = 0; i < 9; i++) {
    ...
    checkSolution: function (posI, posJ) {
        ...
        if (!posI && !posJ) {
            ...
            if (that.emptyCellsCount == 0) {
                that.alertMessage('Congratulation');
                that.finalizeGame();
                return;
            }
        }
